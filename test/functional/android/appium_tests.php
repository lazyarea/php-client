...
define("APP_PATH", realpath(dirname(__FILE__).'/../../apps/app-debug.apk'));
if (!APP_PATH) {
...
//    public function testSetText()
    ...
    {
        $conf = $this->config_loader();
        $this->byId($conf['layouts']['login_form']['email'])->setText($conf['login_user']['email']);
        $this->byId($conf['layouts']['login_form']['password'])->setText($conf['login_user']['password']);
        $this->clickOnElement($conf['layouts']['login_form']['btn_login']);
        $this->assertEquals(1,1);
    }

    public function config_loader($fpath=null){
        $fpath = getcwd().'/test/functional/config.yml';
        return Yaml::parse(file_get_contents($fpath));
    }

    public function dumper($data = null, $fpath = null){
        if(!$fpath){
            $fpath = getcwd() . '/logs/dumper.log';
        }
        error_log("\n",3,$fpath);
